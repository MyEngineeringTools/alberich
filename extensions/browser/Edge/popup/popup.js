...
async function init() {
  ...
  els.btnEncrypt.addEventListener('click', () => {
    showHint('');
    ...
    const result = encryptModern(config, plain);
    if (!result.ok) {
  ...
  els.btnDecrypt.addEventListener('click', () => {
    showHint('');
    const config = keys.getDayConfig();
    if (!config) {
      showHint(t('modern.noKey'));
    ...
    const result = decryptModern(config, cipher);
    if (!result.ok) {
      showHint(errorText(result));
      return;
    }
